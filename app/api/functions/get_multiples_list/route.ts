...
export async function GET(request: Request) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/api/multiples_list`);
    url.searchParams.set("api_key", apiKey);
    url.searchParams.set("tickers", tickers.join(","));

    const { res, text } = await fetchEquipicker(url, "multiples_list", {
    ...
    console.error("Error getting multiples list:", error);
    ...
    return new Response(
      JSON.stringify({
        error:
          error instanceof Error ? error.message : "Error getting multiples list",
      }),
      { status: 500 }
    );
  }
