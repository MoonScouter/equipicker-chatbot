...
export async function GET(request: Request) {
  try {
    ...
    const url = new URL(`${CI_BASE_URL}/api/multiples`);
    url.searchParams.set("api_key", apiKey);
    url.searchParams.set("ticker", ticker);

    const { res, text } = await fetchEquipicker(url, "multiples", { ticker });
    if (!res.ok) {
      return new Response(
    ...
    console.error("Error getting multiples:", error);
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : "Error getting multiples",
      }),
      { status: 500 }
    );
  }
