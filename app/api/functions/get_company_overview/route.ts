...
export async function GET(request: Request) {
  try {
    ...
    const url = new URL("https://ci.equipicker.com/api/company_overview");
    ...
    if (!res.ok) {
      if (process.env.NODE_ENV !== "production") {
        console.error("Equipicker company_overview failed:", {
          status: res.status,
          ticker,
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : "Error getting overview",
      }),
      { status: 500 }
    );
  }
